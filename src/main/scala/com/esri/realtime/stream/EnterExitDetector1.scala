...
object EnterExitDetector1 {
  ...
  case class FeatureTrackUpdater(trackPurger: FeatureTrackPurger, bootstrap: String, topic: String)(implicit val featureSchema: FeatureSchema) {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val featureTrackUpdater = FeatureTrackUpdater(MaxFeaturesPerTrackPurger(2), args(0), args(1))

    val spark = SparkSession
      .builder
      .appName("EnterExitDetector1")
    ...
    val df = spark
      .readStream
      .format("kafka")
      .option("kafka.bootstrap.servers", featureTrackUpdater.bootstrap)
      .option("startingOffsets", "latest")
      .option("subscribe", featureTrackUpdater.topic)
      .load()
